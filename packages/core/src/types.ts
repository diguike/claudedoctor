 ...
 * - `info` — contextual or advisory signal that does not require attention.
 * - `warn` — an actionable access/path concern, incomplete requested probe, or
 *            reported risk. It affects overall health but is not necessarily policy risk.
...
export type SignalId =
  ...
  | 'B4-probe'            // opt-in network probe could not complete
...
export type CredentialKind =
  ...
  | 'cloud-provider'      // Bedrock / Vertex / Foundry (highest documented precedence)
  | 'subscription-oauth'  // official claude.ai login (Keychain / .credentials.json)
  | 'oauth-token-env'     // CLAUDE_CODE_OAUTH_TOKEN, or an OAuth token in the wrong env var
  | 'api-key'             // ANTHROPIC_API_KEY / sk-ant-api* — pay-as-you-go API key
  | 'api-key-helper'      // apiKeyHelper output (value is never inspected)
  | 'auth-token'          // ANTHROPIC_AUTH_TOKEN — documented gateway bearer token
...
/** Optional network probe result (B4). CLI populates it only for `--net`; Web has its own browser-side probe. */
...
export interface DoctorInput {
  ...
  credential: {
    ...
    apiKeyEnvSource: 'process-env' | 'settings-json' | null;
    authTokenEnvSet: boolean;
    authTokenEnvSource: 'process-env' | 'settings-json' | null;
    /** official long-lived token generated by `claude setup-token` */
    oauthTokenEnvSet: boolean;
    oauthTokenEnvSource: 'process-env' | 'settings-json' | null;
    /** documented cloud provider selection, which outranks local credentials */
    cloudProvider: 'bedrock' | 'vertex' | 'foundry' | null;
  ...
  client: {
    version: string | null;
    /** active PATH entry resolves to a recognized Anthropic distribution path */
  ...
  networkProbe?: 'not-requested' | 'complete' | 'failed';
}
...
export interface Fix {
   ...
   * so users know it won't change the policy-risk classification.
   */
...
export interface Finding {
  ...
  /** true = has a real server-side causal link; false = access/path/context evidence. */
  causal: boolean;
  /**
